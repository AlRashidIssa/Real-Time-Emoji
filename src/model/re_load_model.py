...
from utils.logging_utils import model_logger
...
class LoadPreTrainedModel(ILoadPreTrainedModel):
    ...
    def call(self, model_path: str) -> tf.keras.Model:
        ...
        if not os.path.exists(model_path):
            model_logger.error(f"The file does not exist: {model_path}")
        ...
        try:
            ...
            if model is None:
                model_logger.error("The Model is None")
            ...
            model_logger.error(f"Error loading model file: {model_path}. Exception: {str(e)}")
            raise OSError(f"Error loading model file: {model_path}") from e
        
        except Exception as e:
            # Catch any other exceptions that may occur
            model_logger.error(f"An unexpected error occurred while loading the model: {str(e)}")
            raise